...
# =========================================================================
# AI Models & Services — real config-derived status, not fabricated uptime.
# =========================================================================
@router.get("/ai-services", response_model=list[AIServiceStatusOut])
async def ai_services_status(current_user: dict = Depends(require_roles(UserRole.administrator))):
    return [
        AIServiceStatusOut(
            provider="OpenAI",
            configured=bool(settings.OPENAI_API_KEY),
            role="primary" if settings.LLM_PROVIDER == "openai" else "fallback",
        ),
        AIServiceStatusOut(
            provider="Anthropic",
            configured=bool(settings.ANTHROPIC_API_KEY),
            role="primary" if settings.LLM_PROVIDER == "anthropic" else "fallback",
        ),
        AIServiceStatusOut(
        ...
        AIServiceStatusOut(
            provider="OpenAI Whisper (speech-to-text)",
            configured=bool(settings.OPENAI_API_KEY),
            role="primary",
        ),
        AIServiceStatusOut(
