...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      {
        to: "/learner/tools/argument-analyzer",
        label: "Argument Analyzer",
        icon: BrainCircuit,
      },
      {
        to: "/learner/tools/fallacy-detector",
        label: "Fallacy Detector",
        icon: AlertTriangle,
      },
      {
        to: "/learner/tools/counterargument-generator",
        label: "Counterargument Generator",
        icon: Swords,
      },
      {
        to: "/learner/presentation",
    ...
    <div className="flex min-h-screen bg-[#f7f7fb] dark:bg-ink-950">
      ...
      >
        <div className="flex h-16 items-center gap-2 border-b border-black/5 px-4 dark:border-white/10">
          {collapsed ? (
            <span className="font-display text-lg font-black tracking-tight text-brand-500">
              AI
            </span>
          ) : (
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${
                          isActive
                            ? "bg-blue-100 text-blue-700 shadow-premium dark:bg-brand-500 dark:text-white"
      ...
      {/* Main column */}
      <div
        className={`flex min-h-screen flex-1 flex-col transition-all duration-300 ${collapsed ? "md:ml-[76px]" : "md:ml-64"}`}
      >
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-black/5 bg-white/80 px-4 backdrop-blur-lg dark:border-white/10 dark:bg-ink-900/80 sm:px-6">
          ...
          <div className="relative hidden max-w-sm flex-1 sm:block">
            <Search
              size={15}
              className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-900/30 dark:text-white/30"
            />
            <input
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            >
              ...
              <ChevronDown
                size={14}
                className="hidden text-ink-900/40 dark:text-white/40 sm:inline"
              />
            </button>
          </div>
        </header>
