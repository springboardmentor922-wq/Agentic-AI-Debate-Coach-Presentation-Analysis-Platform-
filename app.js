...
(function () {
  ...
  function renderMainView() {
    ...
    if (extensionViews.includes(view)) {
      const html = (window.ExtensionsModule && window.ExtensionsModule.render) ? window.ExtensionsModule.render(view, role) : `<div style="padding:40px;text-align:center;"><h2>Extension Module Not Loaded</h2></div>`;
      main.innerHTML = `<div class="dashboard-container">${renderSidebar()}<div class="dashboard-main" id="main-content-area-ext">${html}</div></div>`;
      if (window.ExtensionsModule && window.ExtensionsModule.bindEvents) {
        window.ExtensionsModule.bindEvents(view, document.getElementById('main-content-area-ext'));
      }
      bindSidebarEvents();
      return;
    }
    ...
    if (view === 'coach-feedback' && role === 'learner') {
      const html = `<div class="dash-header"><div><h1>Coach Feedback 📝</h1><p style="color: var(--text-muted);">View detailed feedback from your coaches here.</p></div></div><div class="glass-panel" style="padding: 24px; margin-top:20px;"><p>You have no new coach feedback at this time.</p></div>`;
      main.innerHTML = `<div class="dashboard-container">${renderSidebar()}<div class="dashboard-main">${html}</div></div>`;
      bindSidebarEvents();
      return;
    }

    if (view === 'learner-directory' && role === 'coach') {
      const html = (window.LearnerDirectoryModule && window.LearnerDirectoryModule.render) ? window.LearnerDirectoryModule.render() : `<div style="padding:40px;text-align:center;"><h2>Learner Directory Not Loaded</h2></div>`;
      main.innerHTML = `<div class="dashboard-container">${renderSidebar()}<div class="dashboard-main" id="main-content-area-dir">${html}</div></div>`;
      if (window.LearnerDirectoryModule && window.LearnerDirectoryModule.bindEvents) {
        window.LearnerDirectoryModule.bindEvents(document.getElementById('main-content-area-dir'));
      }
      bindSidebarEvents();
      return;
    }
    ...
    if (view === 'evaluated-vault' && role === 'coach') {
      const html = (window.EvaluatedVaultModule && window.EvaluatedVaultModule.render) ? window.EvaluatedVaultModule.render() : `<div style="padding:40px;text-align:center;"><h2>Evaluated Vault Not Loaded</h2></div>`;
      main.innerHTML = `<div class="dashboard-container">${renderSidebar()}<div class="dashboard-main" id="main-content-area-vault">${html}</div></div>`;
      if (window.EvaluatedVaultModule && window.EvaluatedVaultModule.bindEvents) {
        window.EvaluatedVaultModule.bindEvents(document.getElementById('main-content-area-vault'));
      }
      bindSidebarEvents();
      return;
    }
    ...
    if (view === 'dashboard') {
      if (role === 'learner') {
        ...
        main.innerHTML = `<div class="dashboard-container">${renderSidebar()}<div class="dashboard-main"><div style="padding:40px;text-align:center;"><h2>Component Not Found</h2><p>The view '${view}' could not be loaded.</p></div></div></div>`;
    }
