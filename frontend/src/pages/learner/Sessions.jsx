...
const POSITIONS_BY_FORMAT = Object.fromEntries(FORMATS.map((f) => [f.value, ['For', 'Against']]))
...
export default function Sessions() {
  ...
  const [format, setFormat] = useState(location.state?.format || FORMATS[0].value)
  const [personality, setPersonality] = useState('intermediate')
  const [position, setPosition] = useState(POSITIONS_BY_FORMAT[FORMATS[0].value][0])
  ...
  const [topicMode, setTopicMode] = useState(location.state?.customMode ? 'custom' : 'curated')
  ...
  useEffect(() => {
    ...
    api.get('/debate/topics', { params: { debate_format: format } })
      .then((res) => { if (!cancelled) setCuratedTopics(res.data) })
      .catch(() => { if (!cancelled) setCuratedTopics([]) })
  ...
  const submitTurnRecording = async (blob, durationSeconds) => {
    ...
    try {
      ...
      const draftRes = await api.post('/debate/transcribe', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      ...
      if (session?.id) {
        ...
        await api.post('/debate/upload-audio', fd2, { headers: { 'Content-Type': 'multipart/form-data' } })
        setLastRecordingNote('Recording saved — your full presentation report will appear in session history shortly.')
      }
    } catch (err) {
      setFormError(err.response?.data?.detail || 'Could not process your recording. You can type your turn instead.')
    } finally {
  ...
  const startSession = async (e) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.detail || 'Could not start the session.')
    } finally {
      setStarting(false)
    }
  }
  ...
  const endSession = async () => {
    ...
    try {
      if (turns.length > 0) {
        ...
        const res = await api.post('/debate/finish', null, { params: { session_id: session.id } })
        setSession(res.data.session)
      } else {
        // Nothing was ever recorded — be honest about it instead of faking
        // a 'completed' debate with no transcript and no report.
        const res = await api.patch(`/debate/sessions/${session.id}/status`, { status: 'cancelled' })
        setSession(res.data)
        setFormError('No turns were recorded, so this session was cancelled rather than marked completed — there was nothing for the AI to analyze.')
      }
    } catch (err) {
  ...
  const pauseSession = async () => {
    ...
    setPausing(true)
    try {
      const res = await api.patch(`/debate/sessions/${session.id}/status`, { status: 'paused' })
      setSession(res.data)
    } catch (err) {
      setFormError(err.response?.data?.detail || 'Could not pause the session.')
    } finally {
      setPausing(false)
    }
  }
  ...
  const resumeSession = async () => {
    ...
    setPausing(true)
    try {
      const res = await api.patch(`/debate/sessions/${session.id}/status`, { status: 'active' })
      setSession(res.data)
    } catch (err) {
      setFormError(err.response?.data?.detail || 'Could not resume the session.')
    } finally {
      setPausing(false)
    }
  }
  ...
  const downloadReport = async (sessionId) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.detail || 'No report is available for this debate yet.')
    } finally {
  ...
  const submitTurn = async (e) => {
    e.preventDefault()
    if (!draft.trim() || !session || sending || session.status === 'paused') return
    ...
    try {
      ...
      setTurns((prev) => [
        ...prev,
        {
          ...
          fallacy: res.data.fallacy_report?.fallacy_detected ? res.data.fallacy_report : null,
        },
      ])
      ...
      setTurns((prev) => [
        ...prev,
        { speaker: 'ai', text: `⚠️ ${err.response?.data?.detail || 'Analysis failed for that turn.'}` },
      ])
    } finally {
      setSending(false)
    }
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        if (!cancelled) setHistoryError(err.response?.data?.detail || 'Could not load debate history.')
      })
      .finally(() => {
  ...
  const formatLabel = (value) => FORMATS.find((f) => f.value === value)?.label || value

  const filteredHistory = useMemo(() => {
    return history.filter((h) => {
      const matchesSearch = h.topic.toLowerCase().includes(search.toLowerCase())
      const matchesFormat = formatFilter === 'All Formats' || h.format === formatFilter
  ...
  const ended = session && (session.status === 'completed' || session.status === 'cancelled')

  return (
    <div className="page-fade flex flex-col gap-6">
      <div>
        <Breadcrumbs items={[{ label: 'Debate Session' }]} />
        <h1 className="font-display text-2xl font-bold text-ink-900 dark:text-white">Debate Session</h1>
        <p className="text-sm text-ink-900/60 dark:text-white/60">Create a new debate, join a live room, or review your history.</p>
      </div>
      ...
      {tab === 'new' && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="glass-card p-6 lg:col-span-1">
            ...
            {formError && (
              <p className="mb-3 rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-600 dark:bg-rose-500/10 dark:text-rose-300">{formError}</p>
            )}
            {!session ? (
              <form onSubmit={startSession} className="flex flex-col gap-3">
                <select
                  ...
                  value={format}
                  onChange={(e) => { setFormat(e.target.value); setTopic(''); setPosition(POSITIONS_BY_FORMAT[e.target.value][0]) }}
                >
                  {FORMATS.map((f) => (
                ...
                <div className="flex overflow-hidden rounded-lg border border-black/10 dark:border-white/15">
                  <button
                    type="button"
                    onClick={() => { setTopicMode('curated'); setTopic('') }}
                ...
                {topicMode === 'curated' ? (
                  <>
                    <select className="input-field" value={topic} onChange={(e) => setTopic(e.target.value)} disabled={topicsLoading}>
                      <option value="">🎲 Let AI pick a curated topic</option>
                      {curatedTopics.map((t) => (
                        <option key={t.id} value={t.title}>{t.title} — {t.category} ({t.difficulty})</option>
                      ))}
                    </select>
                    {!topicsLoading && curatedTopics.length === 0 && (
                      <p className="text-xs text-ink-900/40 dark:text-white/40">No curated topics for this format yet — a random one will still be assigned.</p>
                    )}
                  </>
                ) : (
                  <>
                    ...
                    <p className="text-xs text-ink-900/40 dark:text-white/40">
                      Your custom topic goes through the exact same AI evaluation pipeline as curated topics.
                    </p>
                  </>
                )}
                ...
                <button type="submit" className="btn-primary" disabled={starting}>
                  {starting ? <Loader2 size={15} className="animate-spin" /> : <Play size={15} />} Start Session
                </button>
              </form>
            ) : ended ? (
              ...
              <div className="flex gap-2">
                {paused ? (
                  ...
                  >
                    {pausing ? <Loader2 size={15} className="animate-spin" /> : <PlayCircle size={15} />} Resume
                  </button>
                ) : (
                  <button
                    type="button"
                    ...
                    className="btn-secondary flex-1"
                  >
                    {pausing ? <Loader2 size={15} className="animate-spin" /> : <PauseCircle size={15} />} Pause
                  </button>
                )}
                <button
                  type="button"
                ...
                >
                  {finishing ? <Loader2 size={15} className="animate-spin" /> : <Square size={15} />}
                  {turns.length > 0 ? 'End & Generate Report' : 'End (no turns yet)'}
                </button>
              </div>
            )}
            ...
            {session && !ended && (
              <div className="mt-6 rounded-xl border border-black/5 p-4 dark:border-white/10">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <span className="font-display text-lg font-bold text-brand-600 dark:text-brand-300">{formatTime(seconds)}</span>
                </div>

                <div className="mb-3">
                  <AudioRecorder onSubmit={submitTurnRecording} submitting={submittingRecording} submitLabel="Use This Recording" />
                  {lastRecordingNote && (
                    <p className="mt-2 text-xs font-medium text-verdict-600">{lastRecordingNote}</p>
                  )}
                </div>

                <p className="mb-3 flex items-center gap-2 text-xs text-ink-900/50 dark:text-white/50">
                  Record your turn or type it directly below — each turn is analyzed for fallacies and argument quality in real time.
                </p>
                ...
                {mediaError && (
                  <p className="mb-3 rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-600 dark:bg-rose-500/10 dark:text-rose-300">{mediaError}</p>
                )}
                {mediaResult && (
                  <div className="mb-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">
                    Presentation score: {mediaResult.presentation_score?.overall_score}/100 — full breakdown on the Presentation Analysis page.
                  </div>
                )}

                <p className="text-center text-[11px] text-ink-900/40 dark:text-white/40">
                  Use the "{turns.length > 0 ? 'End & Generate Report' : 'End'}" button above to finish this debate.
                </p>
              </div>
            )}
            ...
            {session && ended && session.status === 'cancelled' && (
              <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
                This session was cancelled — no turns were recorded, so there was nothing for the AI to analyze.
              </div>
            )}
            ...
            {ended && session.status === 'completed' && (
              <div className="mt-6 flex flex-col gap-2 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-xs text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">
                <p>Session ended. Your feedback report has been generated — check the Reports page for your score.</p>
                <button
                  type="button"
                ...
                >
                  {downloadingReport === session.id ? <Loader2 size={13} className="animate-spin" /> : <Download size={13} />} Download Report (PDF)
                </button>
              </div>
            )}
          </div>
          ...
          <div className="glass-card flex flex-col p-6 lg:col-span-2">
            <div className="mb-4 flex items-center justify-between">
              ...
              {session && !ended && <Badge tone={paused ? 'warning' : 'success'}>● {paused ? 'Paused' : live ? 'Live' : 'Scheduled'} · AI Opponent</Badge>}
            ...
            {!session ? (
              ...
              <>
                <p className="mb-4 text-sm text-ink-900/60 dark:text-white/60">
                  Topic: <span className="font-semibold text-ink-900 dark:text-white">{session.topic}</span> · {formatLabel(session.debate_format)}
                  {session.position && <> · You: <span className="font-semibold text-ink-900 dark:text-white">{session.position}</span></>}
                </p>
                <div className="mb-4 flex max-h-96 flex-col gap-3 overflow-y-auto pr-1">
                  {turns.length === 0 && (
                    <p className="text-sm text-ink-900/50 dark:text-white/50">Open with your first argument below — each turn is analyzed for fallacies and argument quality in real time.</p>
                  )}
                  {turns.map((t, i) => (
                    <div key={i} className="flex flex-col gap-1">
                      ...
                      {t.fallacy && (
                        <div className="ml-1 flex items-center gap-1.5 text-xs text-alert-500">
                          <Badge tone="warning">{t.fallacy.fallacy_type}</Badge> {t.fallacy.explanation}
                        </div>
                      )}
                    </div>
                  ))}
                  {sending && <p className="text-xs text-ink-900/40 dark:text-white/40">Analyzing your turn…</p>}
                </div>
                <form onSubmit={submitTurn} className="mt-auto flex gap-2">
                  <input
                    className="input-field"
                    placeholder={paused ? 'Session is paused — resume to continue…' : 'Type your argument (speech transcript auto-fills when recording)…'}
                    value={draft}
                    onChange={(e) => setDraft(e.target.value)}
                    disabled={ended || paused || sending}
                  />
                  <button type="submit" className="btn-primary !px-4" disabled={ended || paused || sending}>
      ...
      {tab === 'history' && (
        <div className="glass-card p-6">
          <Toolbar>
            <SearchInput value={search} onChange={(v) => { setSearch(v); setPage(1) }} placeholder="Search past debates…" />
            <SelectFilter
              value={formatFilter}
              onChange={(v) => { setFormatFilter(v); setPage(1) }}
              options={['All Formats', ...FORMATS.map((f) => f.value)].map((f) => ({ value: f, label: f === 'All Formats' ? f : formatLabel(f) }))}
            />
          </Toolbar>
          ...
          {historyLoading ? (
            <p className="py-8 text-center text-sm text-ink-900/50 dark:text-white/50">Loading your debate history…</p>
          ) : pageItems.length === 0 ? (
            <EmptyState icon={History} title="No debates found" description={history.length === 0 ? 'Complete a debate session to see it here.' : 'Adjust your search or filters.'} />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {pageItems.map((h) => (
                    <tr key={h.id} className="border-b border-black/5 last:border-0 dark:border-white/10">
                      <td className="py-3 pr-4 font-medium text-ink-900 dark:text-white">{h.topic}</td>
                      <td className="py-3 pr-4 text-ink-900/60 dark:text-white/60">{formatLabel(h.format)}</td>
                      <td className="py-3 pr-4 text-ink-900/60 dark:text-white/60">{h.date ? new Date(h.date).toLocaleDateString() : '—'}</td>
                      <td className="py-3 pr-4 text-ink-900/60 dark:text-white/60">{formatDuration(h.duration_seconds)}</td>
                      <td className="py-3 pr-4 font-semibold text-ink-900 dark:text-white">{h.score != null ? `${h.score}/100` : '—'}</td>
                      <td className="py-3 pr-4"><Badge tone={RESULT_TONE[h.result]}>{h.result}</Badge></td>
                      <td className="py-3 pr-4">
                        ...
                        >
                          {downloadingReport === h.id ? <Loader2 size={12} className="animate-spin" /> : <Download size={12} />}
                        </button>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="mt-4">
            <Pagination page={page} totalPages={totalPages} onChange={setPage} totalItems={filteredHistory.length} pageSize={PAGE_SIZE} />
          </div>
        </div>
      )}
    </div>
