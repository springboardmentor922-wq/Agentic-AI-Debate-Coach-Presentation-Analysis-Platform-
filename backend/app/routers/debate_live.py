...
async def live_turn(
    ...
    if session.get("status") not in ("active", "scheduled"):
        raise HTTPException(
            status_code=400,
            detail=f"Session is '{session.get('status')}' and cannot accept turns",
        )

    personality = (
        payload.ai_personality.value
        if payload.ai_personality
        else session.get("ai_personality", "intermediate")
    )

    # ---------------------------------------------
    # Step 1: Analyze user's argument
    # ---------------------------------------------
    argument_report = await analyze_argument(payload.text)

    # ---------------------------------------------
    # Step 2: Run fallacy detection and AI rebuttal
    # in parallel
    # ---------------------------------------------
    ...
    # Wait for both tasks
    fallacy_report = await fallacy_task
    ai_rebuttal = await rebuttal_task
    ...
    await session_transcripts_collection.update_one(
        {"session_id": payload.session_id},
        {
            ...
            "$setOnInsert": {"session_id": payload.session_id},
        },
        upsert=True,
    )
