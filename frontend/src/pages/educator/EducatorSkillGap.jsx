...
export default function EducatorSkillGap() {
  ...
  useEffect(() => {
    api.get('/educator/learners').then(({ data }) => setLearners(data)).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    api.get('/educator/skill-gap', { params }).then(({ data }) => setData(data)).finally(() => setLoading(false))
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-ink-900/60 dark:text-white/60">
            Real scores platform-wide — filter by learner or department, see trends over time.
          </p>
        </div>
      </div>
