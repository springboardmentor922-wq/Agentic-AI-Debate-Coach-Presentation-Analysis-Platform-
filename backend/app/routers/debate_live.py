...
# Topic seeding runs once at app startup (see app/main.py's lifespan
# handler, which calls topics_service.ensure_seeded() directly) rather than
# a router-level @router.on_event("startup") — that API is deprecated in
# FastAPI, and centralizing every startup task in one lifespan function
# also means tests using a real ASGI lifespan (unlike httpx's
# ASGITransport, which never triggers startup at all) only need one hook to
# reason about instead of several scattered across routers.
...
async def live_turn(request: Request, payload: DebateLiveTurnRequest, current_user: dict = Depends(require_roles(UserRole.learner))):
    """
