...
function EducatorNotifications() {
  const [notifications, setNotifications] = useState([]);
  useEffect(() => { api.get("/educator/notifications").then((res) => setNotifications(res.data)).catch(() => {}); }, []);
  ...
  return (
    <Layout>
      ...
      <div className="space-y-3 max-w-2xl">
        ...
        {notifications.length === 0 && <p className="text-gray-500">Nothing pending right now.</p>}
      </div>
    </Layout>
  );
}
