...
function EducatorLearners() {
  ...
  const openNotes = async (learner) => {
    setNotesFor(learner);
    const res = await api.get(`/educator/learner-notes/${learner._id}`).catch(() => ({ data: [] }));
    setNotes(res.data);
  };

  const saveNote = async () => {
    if (!noteTitle.trim()) return;
    await api.post(`/educator/learner-notes/${notesFor._id}`, { title: noteTitle, content: noteContent }).catch(() => {});
    setNoteTitle(""); setNoteContent("");
    const res = await api.get(`/educator/learner-notes/${notesFor._id}`).catch(() => ({ data: [] }));
    setNotes(res.data);
  };

  return (
    <Layout>
      <h2 className="text-2xl font-bold mb-4">Learners</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((l) => (
          <div key={l._id} className="bg-[#1a1a2b] border border-white/5 rounded-2xl p-5">
            ...
            <p className="text-gray-400 text-sm mb-3">Experience: {l.experience}</p>
            <button onClick={() => openNotes(l)} className="text-purple-400 hover:text-purple-300 text-sm font-medium">Add Notes</button>
          </div>
        ))}
      </div>
      ...
      {notesFor && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setNotesFor(null)}>
          <div className="bg-[#1a1a2b] border border-white/10 rounded-2xl p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={saveNote} className="bg-purple-600 hover:bg-purple-700 transition text-white text-sm font-semibold px-4 py-2 rounded-lg mb-4">Save Note</button>
